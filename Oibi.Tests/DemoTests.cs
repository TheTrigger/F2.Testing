...
namespace Oibi.Tests
{
    ...
    public class DemoTests : IClassFixture<ServerFixture<Startup>>
    {
        ...
        /// </summary>
        [Fact]
        public void TestRouteAnalyzer()
        {
            var analyzer = _testFixure.GetService<RouteAnalyzer>();

            var routes = analyzer.GetAllRouteInformations();
            Assert.NotEmpty(routes);

            var path = analyzer.Retrieve<WeatherForecastController>(e => e.Get());
        }
    }
}